...
**Routing**: `HashRouter` (required for static GitHub Pages hosting with no server-side rewrite support) with routes defined in `src/app/routes.tsx`: `/`, `/folders/:folderId`, `/notebooks/:notebookId`, `/notes/:noteId`, `/data-types`, plus a catch-all 404.
